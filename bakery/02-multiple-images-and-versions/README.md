...
As Bakery's name implies, it was originally designed as a wrapper around [Docker's BuildKit Bake tool](https://docs.docker.com/build/bake/). Bake allows for multiple concurrent builds from a shared configuration file. Bakery generates its own temporary configuration file for use with Bake named `.bakery-bake.json`. The contents of this file can be generated using `bakery build --plan` or the file can be retained for viewing after a build using `bakery build --no-clean`.

The plans generated by Bakery can be executed and loaded directly from Docker if desired:
...
Tests can be run without the assistance of Bakery. The commands below would be functionally equivalent to running `bakery build` followed by `bakery run dgoss`. The image should always be built and loaded locally or pulled prior to running tests.
