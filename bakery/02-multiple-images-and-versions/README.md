...
This example demonstrates how to manage multiple image families with multiple versions each using Posit Bakery. It builds multiple versions of two base images for different OS families, Ubuntu and Rocky Linux, showcasing how Bakery handles diverse container configurations in a single project.

Run all command examples from `bakery/02-multiple-images-and-versions/` as the working directory.
...
While this example treats base OSes as a primary component, Bakery can also define them as a secondary dimension, as [Example 4](../04-image-oses) demonstrates.
...
These commands created this example from scratch.
...
You can run tests without Bakery. The commands below are functionally equivalent to `bakery build` followed by `bakery run dgoss`. Always build and load the image locally, or pull it, before running tests.
...
This example uses the Bakery package manager macros to abstract differences between distributions:

**[apt.j2](https://posit-dev.github.io/images-shared/templating.html#apt-package-management)** (for Debian and Ubuntu):
...
**[dnf.j2](https://posit-dev.github.io/images-shared/templating.html#dnf-package-management)** (for RHEL, Rocky, and Fedora):
